    ...
    while last_dynamic_str != cards_data[index]['desc']['dynamic_id_str']:
        ...
        try:
            if (cards_data[index]['desc']['type'] == 64):
                content_list.append(name +'发了新专栏「'+ cards_data[index]['card']['title'] + '」并说： ' +cards_data[index]['card']['dynamic'])
                ...
                if (cards_data[index]['desc']['type'] == 8):
                    content_list.append(name + '发了新视频「'+ cards_data[index]['card']['title'] + '」并说： ' +cards_data[index]['card']['dynamic'])
                    content_list.append('[CQ:image,file='+cards_data[index]['card']['pic']+']')
                else:         
                    if ('description' in cards_data[index]['card']['item']):
                        # 带图新动态
                        content_list.append(name + '发了新动态： ' +cards_data[index]['card']['item']['description'])
                        ...
                        if 'origin_user' in cards_data[index]['card']:
                            ...
                            content_list.append(name+ '转发了「'+ origin_name + '」的动态并说： ' +cards_data[index]['card']['item']['content'])
                        else:
                            #这个是不带图的自己发的动态
                            content_list.append(name+ '发了新动态： ' +cards_data[index]['card']['item']['content'])
